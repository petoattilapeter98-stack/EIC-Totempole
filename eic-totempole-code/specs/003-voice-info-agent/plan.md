...
**Push-to-talk, formatted replies, and a thinking indicator (2026-09-23)**: real demo use of the hands-free design above surfaced three issues, fixed on `fix/voice-assistant-markdown-rendering` (branched off this feature branch, merged back into it): (1) the mic listened continuously even while the assistant's reply was on screen, picking up ambient noise and the reply being read aloud as if it were a new question — `useSpeechRecognition`'s `active` flag is now driven by a press-and-hold talk button (`VoiceConversation.tsx`'s `talking` state) rather than connection status alone, and releasing the button now calls `recognition.stop()` (not `abort()`) so the tail of the utterance the visitor was still speaking is not discarded; (2) the live agent's replies (bullet lists, emphasis, citation-style reference links) were rendered as literal, unrendered text — a new dependency-free markdown-to-React renderer (`markdown.tsx`) now formats them, deliberately without `dangerouslySetInnerHTML` so it cannot become an XSS vector even for an adversarial or prompt-injected reply; (3) there was no feedback between a question being sent and the reply arriving — a "thinking" indicator (`awaitingReply` state, `VoiceConversation.tsx`) now fills that gap. A fourth, unrequested fix was made after being surfaced by live testing while capturing a real reply sample for (2): Direct Line echoes every posted message back with a server-assigned `from.id` that never matches this app's own `KIOSK_VISITOR_ID` — meaning the visitor's own question was being re-rendered a second time as if the assistant had said it. `contracts/direct-line-client-contract.md`'s guarantee 2 previously (incorrectly) claimed the fixed outgoing `from.id` alone was sufficient to filter echoes; it is corrected below. No new dependency and no new infrastructure were introduced by any of this.

**One message per press, auto-scroll, TEKsystems correction (2026-09-24)**, on `fix/voice-assistant-single-send-autoscroll`: (1) Chrome finalizes a recognition result at every pause even in `continuous` mode, and `useSpeechRecognition` called `onFinalTranscript` for each one, so a single hold with a pause posted several messages. The hook now buffers final segments for the whole activation and calls `onFinalTranscript` once, joined, when the stopped recognizer fires `onend` after release (with a 2 s fallback timer in case it never does); see research.md R18 and `contracts/speech-recognition-contract.md` guarantees 3, 4 and 7. (2) The transcript panel already scrolled internally (`overflow-y: auto`) but was never moved; `VoiceConversation.tsx` now pins it to the bottom whenever entries, the thinking indicator, or live text change (research.md R19). (3) A new tab-local `transcriptCorrections.ts` rewrites known "TEKsystems" mishearings before the question is shown, sent, or queued for echo dedup (research.md R20). No new dependency or infrastructure.
...
**Testing**: Vitest 3.x + React Testing Library (jsdom) for `useSpeechRecognition` (fake `SpeechRecognition` constructor), `VoiceConversation` (mocked `directLineClient`), and `AgentPanel` (idle/active state machine, i18n). The existing Playwright browser-mode project (spec 001 research.md R6) is extended to cover this tab's no-scroll assertion for both states. Real Direct Line connectivity and actual speech-recognition accuracy are **not unit-testable** from this codebase — validated manually per quickstart.md against the real agent and a real Chrome instance. **Added 2026-09-23**: `markdown.tsx` has its own pure-function unit tests (`markdown.test.tsx`), including a fixture of an actual reply captured from the live agent, so the renderer is verified against ground truth without needing another live call; `VoiceConversation.test.tsx`'s push-to-talk cases drive the talk button with `fireEvent.pointerDown`/`pointerUp` rather than assuming recognition starts automatically.
...
**Re-run 2026-09-23** for the push-to-talk/markdown/thinking-indicator changes: no principle regresses. III (touch targets) — the reintroduced talk button reuses the existing `--touch-target-min`/`--touch-gap-min` tokens, same as Start/End. V (reliability) — the echo-dedup queue (`pendingSentTexts`) and the switch from `abort()` to `stop()` on release are both still explicitly bounded/torn down (capped at 5 entries; `stop()` still fires `onend` and the effect cleanup still nulls the handlers that matter). VI (no client secret) — unaffected; the markdown renderer and thinking indicator are pure client-side presentation with no new network surface. VII (accessibility) — improved, not just preserved: the thinking indicator uses `role="status"` inside the transcript's existing `aria-live="polite"` region, so it is announced the same way a new transcript line already was.

**Re-run 2026-09-24** for one-message-per-press, auto-scroll, and the TEKsystems correction: no principle regresses. II (no scroll): no page scroll is introduced; only the existing contained transcript panel's scroll position is set. V (reliability): the per-press segment buffer lives in the recognition effect's closure and is dropped with it; the one new timer (the 2 s fallback flush) is cleared by the flush itself and fires at most once per press. IX (isolation): `transcriptCorrections.ts` is tab-local and imported only by `VoiceConversation.tsx`.
...
│           ├── strings.ts                     # Tab-owned EN/HU copy -- sign-in strings removed, listening/mic-error/send-error strings added (2026-09-10); `waiting` renamed `holdToTalk`, `thinking` added (2026-09-23)
│           ├── examplePrompts.ts               # Static 3-domain example question list (FR-016); employees-domain example text updated 2026-09-23
...
│           ├── useSpeechRecognition.ts         # Chrome SpeechRecognition wrapper (2026-09-10, new); `active` now press-gated and cleanup uses stop() not abort() (2026-09-23); final segments buffered and sent once per press (2026-09-24)
│           ├── useSpeechRecognition.test.ts
│           ├── markdown.tsx                    # Dependency-free markdown-to-React renderer for assistant replies (2026-09-23, new)
│           ├── markdown.test.tsx
│           ├── transcriptCorrections.ts        # Known-mishearing corrections, e.g. "tax systems" -> "TEKsystems" (2026-09-24, new)
│           ├── transcriptCorrections.test.ts
│           ├── VoiceConversation.tsx           # Connects Direct Line + speech recognition; renders transcript (2026-09-10, replaces AgentEmbedContainer.tsx); push-to-talk button, thinking indicator, echo-dedup, markdown rendering added (2026-09-23); transcript auto-scroll and transcript correction added (2026-09-24)
