...
**Input**: User description: "I want to integrate a multi-agent voice information subpage to the main screen, developing on a separate branch. This voice agent will act as an information provider to customers, answering questions regarding our innovation centre, company and employees." Amended 2026-09-09: "The agent integrated by copilot is now published with microsoft authentication. What I want to do is have a login possibility to my (or someone's) microsoft account when entering the voice-info-agent. make this a pop-up because the whole functionality is worthless without it." Amended 2026-09-10: "There is a new copilot agent deployment ... we will deploy the agent without auth ... include voice speech to text ... integrate this with chrome's built in STT ... I do not want the user to have [to] press any button while discussing with the agent." Sign-in is no longer required (see Clarifications, Session 2026-09-10) — User Story 1 below replaces the sign-in story it superseded. Amended 2026-09-23, after real demo use of the hands-free design: "During the demo things might get noisy, so most likely a push-to-talk solution would be better to make sure we only record what we want"; "the response given by the agent that is showed in the UI is in an ugly markdown format. Give proper formatting"; "While the user waits for the response the[re] should be an indicator that the agent is working in the background." Push-to-talk reverses the 2026-09-10 hands-free decision (see Clarifications, Session 2026-09-23, Q6); the other two are additive. A fourth item raised the same session — Chrome's speech recognition frequently mishearing "TEKsystems" — was discussed but explicitly left unimplemented at the user's direction; see Clarifications, Session 2026-09-23. Amended 2026-09-24: "the STT recognition should continue to record until the user presses the hold to talk button, and finishes listening when it released, and only sends one request per button press-release"; "The chat window should automatically scroll down to he newest part of the conversation. Currently it hangs on the top always."; and the deferred "TEKsystems" mishearing is now fixed with a client-side correction (see Clarifications, Session 2026-09-24).

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Visitor has a voice conversation using push-to-talk (Priority: P1) — REPLACES the sign-in story (Session 2026-09-10); listening model reverted to push-to-talk (Session 2026-09-23)

**Superseded (2026-09-10)**: the original User Story 1 ("Someone signs in with a Microsoft account") no longer applies. The Copilot Studio agent was re-deployed 2026-09-10 with Web Channel Security's "Require secured access" OFF, so no sign-in of any kind — operator or visitor — is needed for the assistant to answer questions. See Clarifications, Session 2026-09-10, for what replaced it and why.

**Updated (2026-09-23)**: the text this story carried from 2026-09-10 described continuous, hands-free listening for the entire session. Real demo use surfaced a problem that design didn't anticipate: the mic kept capturing audio while the assistant's reply was on screen, so lobby noise — or someone reading the reply aloud — could be picked up as if it were the next question. The listening model is reverted to push-to-talk: the visitor holds a talk button while speaking each question and releases it when done. Everything else about this story (no sign-in, no on-screen keyboard, Start/End as the only session-level controls) is unchanged. See Clarifications, Session 2026-09-23, Q6.

A visitor walks up, taps Start once, then for each question holds the talk button, speaks, and releases it; the assistant's reply appears on screen. No sign-in, no on-screen keyboard, no third-party popup — the only controls in the whole interaction are Start, End, and the talk button itself.

**Why this priority**: This is the foundational interaction model the rest of the feature depends on — every other user story (asking about the Innovation Centre, the company, employees) assumes this listen/send/receive loop already works. It replaces the sign-in story as the highest-priority requirement because, unlike sign-in, it is not an external configuration step — it is this application's own core interaction contract with the visitor.

**Independent Test**: Open the voice information section, tap Start, hold the talk button and ask a question, release it, receive and read the assistant's on-screen reply, hold the talk button again to ask a follow-up, then tap End — confirm Start, End, and the talk button are the only controls ever needed.

**Acceptance Scenarios**:

1. **Given** the voice information section is idle, **When** the visitor taps Start, **Then** the section reaches a ready state showing a talk button, and the mic does not capture any audio until that button is held (Session 2026-09-23).
2. **Given** the assistant is ready, **When** the visitor holds the talk button and speaks a question, **Then** the section recognizes the utterance while the button is held, and releasing the button sends the recognized question to the assistant automatically — the visitor never taps a separate "send" control.
3. **Given** the assistant has just replied, **When** the visitor holds the talk button again to ask a follow-up, **Then** the section recognizes and sends the follow-up the same way, with no need to re-tap Start.
...
- What happens when the visitor releases the talk button having said nothing, or too quietly for the microphone to pick up anything? Nothing is sent — the section simply returns to its ready state; the visitor can hold the talk button and try again (Session 2026-09-23).
- What happens when background noise in the lobby (other conversations, ambient sound) interferes with recognition? Push-to-talk (Session 2026-09-23) bounds how much ambient noise can reach the microphone at all, since it only captures while the talk button is genuinely held; a misrecognized or garbled question is still possible while held, and if that produces an off-topic or unhelpful reply, the visitor can simply hold the talk button and ask again.
- What happens when the visitor pauses mid-question while still holding the talk button? Nothing is sent during the pause; recording continues until release, and everything said during the whole hold is sent as one question (FR-027, Session 2026-09-24).
- What happens if the visitor releases and immediately presses the talk button again, before the first press's speech has finished processing? Each press is still its own single question: the first press's text is sent once when its processing completes, and the new press starts an independent recording (FR-027, Session 2026-09-24).
- What happens if the assistant takes a few seconds to reply? Per FR-025, the section MUST show a working/thinking indicator for the whole wait, so the visitor can tell the question was received and is being processed rather than assuming the section has stalled.
...
- **FR-004** **(reverted 2026-09-23 to push-to-talk; was continuous/hands-free listening 2026-09-10 through 2026-09-22)**: The mic MUST only capture audio while the visitor is actively holding a dedicated talk control — no continuous or always-on listening. Releasing the talk control MUST send whatever was recognized while it was held. The talk control, Start, and End are the only three controls in this feature; all MUST meet the kiosk's minimum touch target sizing and spacing (Constitution Principle III). This reverses the 2026-09-10 hands-free decision after real demo use showed continuous listening picked up ambient noise and the assistant's own on-screen reply being read aloud, as if either were a new question (see Clarifications, Session 2026-09-23, Q6). **Clarified 2026-09-24**: each press-and-release sends exactly one message; a pause while the control is still held never sends anything (see FR-027, Clarifications Session 2026-09-24 Q9).
...
- **FR-025** *(new 2026-09-23)*: From the moment a recognized question is sent to the assistant until its reply is received (or a failure occurs), the section MUST show a clear "working" indicator, so the visitor knows the assistant is composing an answer rather than the section having gone silent or stalled.
- **FR-026** *(new 2026-09-23)*: On-screen answer text MUST be rendered legibly formatted — paragraphs, lists, emphasis, and links rendered as such — rather than as raw, unrendered markup; a visitor reading a reply MUST NOT see literal formatting syntax (e.g. `**`, `•`, or raw link/citation markup) as part of the answer text.
- **FR-027** *(new 2026-09-24)*: Everything recognized between pressing and releasing the talk control MUST be combined and sent to the assistant as a single message, once, after release. A pause in speech while the control is still held MUST NOT send anything, and no message is sent while the control is still held. If nothing was recognized, nothing is sent.
- **FR-028** *(new 2026-09-24)*: Whenever a new transcript entry, the working indicator (FR-025), or live in-progress recognition text appears, the transcript area MUST show the newest content without the visitor having to scroll it. This governs the position of the transcript area's own contained scroll (see the Edge Case on long transcripts: content may "scroll only within its own contained area"); it does not relax FR-006/SC-004, and the kiosk page itself still never scrolls.
- **FR-029** *(new 2026-09-24)*: Known misrecognitions of the company name "TEKsystems" (e.g. "tax systems", "tech systems", "TEK systems") MUST be corrected to "TEKsystems" in a recognized question before it is shown in the transcript or sent to the assistant, so the shown and sent text always match.
...
- **SC-009** **(replaced again 2026-09-23, was hands-free-only touch scope 2026-09-10)**: In 100% of tested conversations from Start to End, the visitor is never required to touch the screen for any reason other than the Start tap, the End tap, and holding the talk button for each question (Session 2026-09-23, Q6).
...
### Session 2026-09-23 (amendment — push-to-talk reinstated, formatted replies, a thinking indicator)

Following real demo use of the 2026-09-10 hands-free design, the user raised three issues in one session and directed fixes for two of them, with the third handled as an advisory-only discussion:

- **Q6 (supersedes Q4)**: How does a visitor start and stop listening? → A: **Push-to-talk again** — the mic captures only while the visitor holds a talk button, exactly reversing Q4's 2026-09-10 answer. Reason given directly by the user: "During the demo things might get noisy, so most likely a push-to-talk solution would be better to make sure we only record what we want" — continuous listening was also observed picking up the assistant's own on-screen reply as if it were a new question. See FR-004, User Story 1.
- **Q7**: How should the assistant's answer text be displayed? → A: **Rendered/formatted, not raw markdown.** The user reported the on-screen reply looked "in an ugly markdown format" — the live agent's replies use bullet lists, emphasis, and citation-style reference links, previously shown as literal, unrendered syntax. See FR-026.
- **Q8**: How does the visitor know the assistant is working on an answer? → A: **A visible "thinking" indicator**, shown from the moment a question is sent until the reply arrives. Direct quote: "While the user waits for the response the[re] should be an indicator that the agent is working in the background." See FR-025.

**Discussed but explicitly not implemented this session**: the user also reported that Chrome's speech recognition frequently mishears "TEKsystems" as "tax systems," and asked for solutions while directing that no code change be made for it ("Do not make changes regarding the voice recognition just offer solutions to that"). This is a speech-recognition-accuracy problem, not a spec-level behavior change — no requirement above governs it, and no fix is recorded as shipped. Options discussed (phonetic coaching for presenters, a client-side transcript correction pass, Copilot Studio-side synonym training) are recorded in research.md as a deferred item. **Resolved 2026-09-24**: see Session 2026-09-24, Q11.

### Session 2026-09-24 (amendment: one message per press, auto-scrolling transcript, TEKsystems correction)

- **Q9**: When is a recognized question sent? → A: **Exactly once per press-and-release, after release**, containing everything said during the hold. Previously each pause while holding produced its own message, because the browser finalizes recognition at every pause. User's words: "the STT recognition should continue to record until the user presses the hold to talk button, and finishes listening when it released, and only sends one request per button press-release." See FR-004 (clarified), FR-027.
- **Q10**: Where should the transcript be positioned as the conversation grows? → A: **Automatically at the newest content.** User's words: "The chat window should automatically scroll down to he newest part of the conversation. Currently it hangs on the top always." See FR-028.
- **Q11 (resolves the 2026-09-23 deferred item)**: How is "TEKsystems" being heard as "tax systems" handled? → A: **A client-side correction of known mishearings, applied before the question is shown or sent.** Chosen by the user from three options (client-side correction; correction plus browser phrase-biasing; proposal only). Bot-side synonym training in Copilot Studio remains a recommended complementary step outside this codebase. See FR-029, research.md R20.
