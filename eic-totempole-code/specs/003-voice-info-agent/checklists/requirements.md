...
- **2026-09-24 amendment**: the user directed one message per talk-button press (FR-027, clarifying FR-004), an auto-scrolling transcript (FR-028), and a fix for the deferred "TEKsystems" mishearing via client-side correction (FR-029). Re-checked against this checklist: still passes all items above. FR-029 names concrete example mishearings only to make the requirement testable, not to prescribe a mechanism. FR-028 is explicitly scoped so it does not conflict with FR-006/SC-004's no-page-scroll guarantee.
