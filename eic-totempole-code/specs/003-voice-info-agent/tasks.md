...
## Phase 10: Push-to-talk reinstated, formatted replies, a thinking indicator (spec.md 2026-09-23 amendment)

**Trigger**: user feedback after real demo use of the 2026-09-10 hands-free design -- "During the demo things might get noisy, so most likely a push-to-talk solution would be better to make sure we only record what we want"; "the response given by the agent that is showed in the UI is in an ugly markdown format. Give proper formatting"; "there should be an indicator that the agent is working in the background." A fourth item (Chrome mishearing "TEKsystems" as "tax systems") was raised and discussed but explicitly left unimplemented ("Do not make changes regarding the voice recognition just offer solutions to that").

**Goal**: the mic only captures while a talk button is held; assistant replies render as formatted content instead of raw markdown; a visible indicator fills the gap between a question being sent and the reply arriving. Work done on branch `fix/voice-assistant-markdown-rendering` (branched off `feat/voice-info-agent`, later merged back into it and opened as its own PR).

**Numbering note**: continues this file's existing convention (see the top-of-file numbering note) of not renumbering completed, already-verified task history.

- [X] T068 Merged `main` into `feat/voice-info-agent` first (separate from this amendment) to resolve a since-diverged merge conflict in the shared tab registry/nav/context files -- see the branch's own merge-commit history; not itself part of this amendment's scope, recorded here only because it's the commit this phase's work sits on top of.
- [X] T069 Live-probed the deployed agent directly (3 round trips against the real Direct Line conversation, deliberately minimized) to capture its actual reply format rather than guessing it. Found `•`-prefixed bullets, inline `[1]` citations, and a raw reference-link definition footer (`[1]: cite:1 "...docx"`) -- and, as an unplanned second finding from the same probe, that Direct Line's echo of a posted message carries a server-assigned `from.id`, never the client-supplied `KIOSK_VISITOR_ID`. Recorded as research.md R15/R16.
- [X] T070 Created `src/tabs/voice-assistant/markdown.tsx` + `markdown.test.tsx`: a dependency-free markdown-to-React renderer (paragraphs, bullet/numbered lists, bold/italic/code, links, reference-style citations) with no `dangerouslySetInnerHTML`, tested against synthetic cases and the real captured reply sample. Per FR-026.
- [X] T071 `VoiceConversation.tsx`: assistant transcript lines now render through `renderMarkdown()` instead of raw text; the outer transcript-line element changed from `<p>` to `<div>` since markdown output can contain block-level children. `VoiceConversation.module.css` gained nested-element styles (`p`/`ul`/`ol`/`li`/`strong`/`code`/`a`/`sup`) scoped under `.assistantLine`.
- [X] T072 `VoiceConversation.tsx`: fixed the Direct Line echo bug found in T069 -- outgoing text is queued (`pendingSentTexts`, capped at 5) when posted, and the first incoming activity matching it is recognized as the echo and dropped, remembering its `from.id` (`knownSelfIds`) so later echoes from that id are dropped without another text match. `direct-line-client-contract.md` guarantee 2 corrected; `directLineClient.ts`'s own (now-documented-as-ineffective) `from.id` check left in place as a harmless no-op. Per research.md R16.
- [X] T073 Reinstated push-to-talk (spec FR-004, Clarifications Q6): `VoiceConversation.tsx` gained `talking` state driven by `onPointerDown`/`onPointerUp`/`onPointerLeave`/`onPointerCancel` on a new talk button (replacing the old passive "listening indicator"), and `useSpeechRecognition`'s `active` option is now `status === 'ready' && talking`. `useSpeechRecognition.ts`'s cleanup path switched from `abort()` to `stop()` (with `onresult` deliberately left attached through cleanup) so the tail of an utterance isn't dropped on button release; `speech-recognition-contract.md` guarantees 2/3 corrected. `VoiceConversation.module.css` gained `.talkButton`/`.talkButtonActive` (touch-target-min sized, `touch-action: none`, `user-select: none`).
- [X] T074 [P] Updated `src/tabs/voice-assistant/strings.ts`: `waiting` renamed `holdToTalk` (new copy, both locales) and a new `thinking` string added (both locales), reflecting the interaction-model and indicator changes.
- [X] T075 [P] Replaced the "Who leads the engineering team?" example prompt (employees domain) with "How does TEKsystems differentiate itself from its competitors?" in `src/tabs/voice-assistant/examplePrompts.ts` (both locales) -- a content change requested directly, not derived from a spec requirement.
- [X] T076 Rewrote the push-to-talk-dependent cases in `VoiceConversation.test.tsx` (`fireEvent.pointerDown`/`pointerUp` instead of assuming recognition starts automatically) and added new cases: release stops the underlying recognizer and a later press starts a fresh one; the Direct Line echo is not rendered as a duplicate assistant line; markdown renders as real elements in a bot reply. Updated `useSpeechRecognition.test.ts`'s cleanup test for `stop()` not `abort()`, and added a case proving a final result delivered after cleanup still reaches the caller.
- [X] T077 Added the thinking indicator (spec FR-025): `VoiceConversation.tsx` gained `awaitingReply` state (set on send, cleared on a genuine reply or a send failure), rendered as a small `role="status"` bubble with an animated (reduced-motion-aware) dot indicator. Two new `VoiceConversation.test.tsx` cases cover it appearing/clearing around a reply and clearing on send failure.
- [X] T078 Ran `npm run typecheck`, `npm run test` (237/237, up from 220 pre-amendment), `npm run test:layout` (29/29), and `npm run build` after each of T070/T071-072/T073-077 -- all clean throughout, not just at the end.
- [X] T079 [P] Updated `spec.md`: User Story 1 and FR-004 rewritten in place for push-to-talk (tagged superseded/reverted, per this project's convention); added FR-025 (thinking indicator) and FR-026 (formatted text); updated the background-noise/silence Edge Cases and added a reply-latency one; superseded SC-009's touch-scope wording; added Clarifications Session 2026-09-23 (Q6-Q8, plus the deferred TEKsystems item).
- [X] T080 [P] Updated `plan.md`: Summary addendum describing all four changes (including the unplanned echo fix); Technical Context Testing bullet; Constitution Check re-run note; Project Structure tree updated for `markdown.tsx`/`markdown.test.tsx` and the touched files.
- [X] T081 [P] Updated `research.md`: added R15 (live reply format + safe-rendering decision), R16 (the echo/`from.id` correction), R17 (push-to-talk reinstated + `stop()` vs `abort()`), and a "Deferred" section recording the TEKsystems options considered but not implemented.
- [X] T082 [P] Updated `data-model.md`: documented `talking`/`awaitingReply` local state and the `pendingSentTexts`/`knownSelfIds` echo-dedup refs; noted assistant-entry markdown rendering; corrected a pre-existing stale `reset()`/`useKiosk()` naming reference (unrelated to this amendment but found while editing this file).
- [X] T083 [P] Corrected `contracts/direct-line-client-contract.md` guarantee 2 and `contracts/speech-recognition-contract.md` guarantees 2/3 (and their purpose paragraphs) to match the shipped behavior instead of the disproven/superseded claims.
- [X] T084 [P] Rewrote `quickstart.md` Scenario 2 for push-to-talk/thinking-indicator/formatted-reply expectations; corrected Scenario 4's stale `abort()` reference and Scenario 8's stale test-coverage description.
- [X] T085 [P] Updated `checklists/requirements.md` re-confirming the checklist still passes after this amendment.

**Checkpoint**: Push-to-talk, formatted replies, the thinking indicator, and the Direct Line echo fix are implemented, tested (237/237 unit, 29/29 layout, clean build), and documented across every spec-kit artifact. The TEKsystems mis-hearing issue remains open by explicit user direction (research.md's "Deferred" section) -- not incomplete work, a deliberately deferred one.

---

## Phase 11: One message per press, auto-scrolling transcript, TEKsystems correction (spec.md 2026-09-24 amendment)

**Trigger**: user request, three items: "the STT recognition should continue to record until the user presses the hold to talk button, and finishes listening when it released, and only sends one request per button press-release"; "The chat window should automatically scroll down to he newest part of the conversation. Currently it hangs on the top always."; and, for the Phase 10 deferred item, "'TEK' or 'TEKSystems' ends up being recognized as tax systems. Propose a solution for this issue." The user then chose the client-side correction option. "Make sure all of the changes are aligned with the speckit workflow."

**Goal**: one talk-button press sends exactly one message, however many pauses it contains. The transcript panel keeps the newest content in view. Known "TEKsystems" mishearings are corrected before a question is shown or sent. Work done on branch `fix/voice-assistant-single-send-autoscroll` (branched off `fix/voice-assistant-markdown-rendering`).

- [X] T086 `useSpeechRecognition.ts`: final segments are buffered per activation (`finalSegments`, closure-scoped) instead of delivered immediately. The live text shows buffered finals plus interim. On teardown, `onend` stays attached and flushes the joined text once via an idempotent `flush()`, with a 2 s fallback timer that `flush()` clears. A mid-hold `onend` restarts and keeps the buffered text on screen. Per FR-027, research.md R18.
- [X] T087 `VoiceConversation.tsx`: added `transcriptRef` on the `.transcript` panel and an effect that sets `scrollTop = scrollHeight` when entries, `awaitingReply`, live text, or status change. Per FR-028, research.md R19.
- [X] T088 [P] Created `src/tabs/voice-assistant/transcriptCorrections.ts` (`correctTranscript()`, a fixed confusables list; first entry maps "tax/tech/tek/… systems" to "TEKsystems") and applied it in `VoiceConversation.tsx`'s `onFinalTranscript` before display, posting, and echo-dedup queuing. Per FR-029, research.md R20.
- [X] T089 Tests. `useSpeechRecognition.test.ts`: replaced the per-result callback cases with (a) finals buffered and shown as live text but not sent while active, (b) segments across a mid-hold restart plus the post-`stop()` trailing final sent once, joined, on `onend`, (c) nothing sent for an empty activation, (d) the fallback timer flushing once when `onend` never fires, with a late `onend` not sending again. `VoiceConversation.test.tsx`: the fake recognizer's `stop()` now fires `onend` asynchronously like Chrome; added cases for one message per press across a mid-hold pause, the TEKsystems correction reaching both the transcript and the post, and the transcript scrolling to the newest reply. New `transcriptCorrections.test.ts`: 6 corrected variants, 5 unchanged inputs.
- [X] T090 Ran `npm run typecheck`, `npm run test` (377 passed, 1 skipped), `npm run test:layout` (55 passed, 1 skipped), and `npm run build`. All clean. (The totals differ from Phase 10's 237/29 mainly because `main`, with other features' tests, was merged in since then.)
- [X] T091 [P] Updated `spec.md`: Input line; FR-004 clarified; new FR-027 (one message per press), FR-028 (auto-scroll, scoped against FR-006/SC-004), FR-029 (TEKsystems correction); two new Edge Cases (mid-hold pause, quick re-press); Clarifications Session 2026-09-24 (Q9–Q11), with the 2026-09-23 deferred item marked resolved.
- [X] T092 [P] Updated `plan.md`: Input line, Summary addendum, Constitution Check re-run (II, V, IX), and the Project Structure tree for `transcriptCorrections.ts`/`.test.ts`.
- [X] T093 [P] Updated `research.md`: added R18 (buffer and flush on `onend`), R19 (auto-scroll), R20 (correction list plus the recommended Copilot Studio synonyms), and marked the Deferred section resolved.
- [X] T094 [P] Updated `contracts/speech-recognition-contract.md`: guarantee 3 amended, guarantee 4 rewritten (once per activation), guarantee 7 added (fallback flush).
- [X] T095 [P] Updated `data-model.md` (per-press buffer, transcript ref, corrected visitor text), `quickstart.md` Scenario 2 steps 13–15, and `checklists/requirements.md` (re-confirmed).
- [ ] T096 Manual verification in real Chrome at 1920×1280 per quickstart.md Scenario 2 steps 13–15 (pause mid-hold, auto-scroll, TEKsystems in EN and HU). Not unit-testable: needs a real microphone and Chrome's live speech service.
- [ ] T097 (Outside this repo, recommended) Add "tax systems" and "tech systems" as synonyms for TEKsystems in the Copilot Studio agent (research.md R20).

**Checkpoint**: one-message-per-press, auto-scroll, and the TEKsystems correction are implemented, unit-tested, and documented across every spec-kit artifact. T096 (live Chrome check) and T097 (bot-side synonyms) remain open.
